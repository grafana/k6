...
usage() {
  this=$1
  cat <<EOF
$this: download go binaries for loadimpact/k6
...
execute() {
  ...
  install -d "${BINDIR}"
  install "${TMPDIR}/${NAME}/${BINARY}" "${BINDIR}/"
...
adjust_format() {
  #it is executed before adjunsting os
...
CHECKSUM=${BINARY}-v${VERSION}-checksums.txt
