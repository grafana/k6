...
func NewOldEngine(r Runner) (*OldEngine, error) {
	e := &OldEngine{
		Runner: r,
		Status: Status{
...
func (e *OldEngine) Apply(opts Options) error {
...
func (e *OldEngine) Run(ctx context.Context) error {
	...
	if e.Collector != nil {
		...
		go func() {
			...
			log.Debug("OldEngine: Collector shut down")
			e.waitGroup.Done()
		}()
	} else {
		log.Debug("OldEngine: No Collector")
	}

	e.waitGroup.Add(1)
	go func() {
		e.runThresholds(subctx)
		log.Debug("OldEngine: Thresholds shut down")
		e.waitGroup.Done()
	}()

	e.consumeOldEngineStats()
...
loop:
	for {
		select {
		case now := <-ticker.C:
			// Don't tick at all if the OldEngine is paused.
			...
			if stage.StartVUs.Valid && stage.EndVUs.Valid {
				...
				if err := e.SetVUs(vus); err != nil {
					log.WithError(err).Error("OldEngine: Couldn't interpolate")
				}
			}
	...
	log.Debug("OldEngine: Waiting for subsystem shutdown...")
...
func (e *OldEngine) IsRunning() bool {
	return e.ctx != nil
}

func (e *OldEngine) TotalTime() (total time.Duration, finite bool) {
...
func (e *OldEngine) SetRunning(running bool) {
	if running && !e.Status.Running.Bool {
		e.Pause.Done()
		log.Debug("OldEngine Unpaused")
	} else if !running && e.Status.Running.Bool {
		e.Pause.Add(1)
		log.Debug("OldEngine Paused")
	}
	e.Status.Running.Bool = running
}

func (e *OldEngine) SetVUs(v int64) error {
	...
	for i := current; i < v; i++ {
		...
		go func() {
			...
			log.WithField("id", id).Debug("OldEngine: VU terminated")
			e.waitGroup.Done()
		}()
		e.nextID++
	}
	for i := current - 1; i >= v; i-- {
		log.WithField("id", i).Debug("OldEngine: Terminating VU...")
		entry := e.vus[i]
		entry.Cancel()
	}
...
func (e *OldEngine) SetMaxVUs(v int64) error {
...
func (e *OldEngine) runVU(ctx context.Context, id int64, vu *vuEntry) {
	if vu.VU == nil {
		return
	}
...
func (e *OldEngine) runThresholds(ctx context.Context) {
...
func (e *OldEngine) getSink(m *stats.Metric) stats.Sink {
	s, ok := e.Metrics[m]
	if !ok {
		switch m.Type {
		case stats.Counter:
...
func (e *OldEngine) consumeBuffer(buffer []stats.Sample) {
...
func (e *OldEngine) consumeOldEngineStats() {
	t := time.Now()
