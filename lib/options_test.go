...
func TestOptions(t *testing.T) {
	...
	t.Run("BlockedHostnames", func(t *testing.T) {
		...
		_, matches := opts.BlockedHostnames.Contains("K6.Io")
		assert.False(t, matches)
		match, matches := opts.BlockedHostnames.Contains("tEsT.k6.Io")
		assert.True(t, matches)
		assert.Equal(t, "test.k6.io", match)
		match, matches = opts.BlockedHostnames.Contains("TEST.K6.IO")
		assert.True(t, matches)
		assert.Equal(t, "test.k6.io", match)
		match, matches = opts.BlockedHostnames.Contains("blocked.valId.paTtern")
		assert.True(t, matches)
		assert.Equal(t, "*valid.pattern", match)
		_, matches = opts.BlockedHostnames.Contains("example.test.k6.io")
		assert.False(t, matches)
		assert.NoError(t, opts.BlockedHostnames.Insert("*.test.k6.io"))
		match, matches = opts.BlockedHostnames.Contains("example.test.k6.io")
		assert.True(t, matches)
		assert.Equal(t, "*.test.k6.io", match)
	})
