...
// SetTag adds the given key=value tag to the Tags and removes it from the
// Metadata, if it was present there.
...
// SetMetadata adds the given key=value datum to the Metadata and removes it
// from the Tags, if it was present there.
...
// Delete removes the given key from both the tags and the metadata.
func (tm *TagsAndMeta) Delete(key string) {
	tm.Tags = tm.Tags.Without(key)
	if tm.Metadata != nil {
		delete(tm.Metadata, key)
	}
