...
Every scenario below is written as a test vector: the **GIVEN** fixes the catalog data (and any relevant build state), the **WHEN** fixes the script and the k6 command, and the **THEN** fixes the expected usage report. These vectors are the rows of two table-driven integration tests, one per command surface (`k6 run`, `k6 x`), and nothing is mocked (see the testing requirement at the end).
...
#### Scenario: Private import is omitted
- **GIVEN** the stand-in catalog does not list the module path behind `k6/x/secret`
- **WHEN** `k6 run` executes a script that imports `k6/x/secret`
...
The recorded usage for each surface is a name, not a module path: an import specifier (`k6/x/sql`), an output type (`--out <name>`), or a subcommand name. The system MUST resolve each recorded name to its Go module path through the extension registry (every registered extension carries both its name and its module path, read from build info) before the lookup. Matching MUST be by module path, never by the recorded name, so a private fork that reuses a public import name resolves to its own module path and is dropped.
...
- **AND** a marker the subcommand body writes is present, proving the wrapped run hook still ran
