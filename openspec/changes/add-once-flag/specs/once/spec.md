...
Passing the bare `--once` flag MUST configure one effective `shared-iterations` scenario with `vus: 1` and `iterations: 1`, without requiring a script edit.

#### Scenario: Primary run commands apply the same single-run configuration
...
- **WHEN** the user passes bare `--once` through each primary run command:
...
When the effective configuration contains one scenario, single-run mode MUST keep that scenario. A preserved non-empty `exec` value MUST select that exported function. A missing `exec` value or `exec: 'default'` MUST select the default function.
...
Single-run mode MUST preserve `options.browser` on a declared scenario and MUST keep the scenario name used to find that configuration.

#### Scenario: A browser scenario runs once with its configuration preserved
...
### Requirement: Supported run inputs use the same single-run mode

`--once` MUST be accepted by `k6 run` and `k6 cloud run`, including `k6 cloud run --local-execution`, for script, archive, and standard-input sources those commands already support.
...
### Requirement: Single-run mode validates the effective configuration

Single-run mode MUST validate the fresh scenario instead of discarded load-shaping values. Source fields MUST still decode, a declared scenario MUST name a known executor and contain only known fields, and the function selected by the effective scenario MUST exist.
...
#### Scenario: Loading and structural errors still fail
...
### Requirement: Single-run mode does not invent browser configuration

Single-run mode MUST NOT add browser configuration to a scenario that did not declare it.
...
- **AND** `api` prints whether its script-visible scenario has `options.browser` from `k6/execution`
...
### Requirement: Single-run mode is enabled only by a bare CLI flag

`--once` MUST be disabled by default. Only the bare flag on the current invocation may enable single-run mode. This proposal defines no value form and no scenario selector for the flag.

An exported script option, `K6_ONCE`, `ONCE`, a JSON configuration field, or an archive option named `once` MUST be inert. Single-run mode MUST NOT become a `Config` or `lib.Options` field, and archives created for a single run MUST carry the behavior through their scenario configuration instead of a stored flag.
