...
The system MUST provide a `k6 features` subcommand that prints a table of all currently registered flags. The table MUST show three columns for every flag — feature, lifecycle, and description — so the command is self-documenting and requires no other documentation source to convey what flags exist and what they do. Default sort: grouped by lifecycle in the order `Experimental` → `Deprecated` → `GA` (most-actionable first), alphabetical by feature within each group. The lifecycle column uses Title Case display (`Experimental`/`GA`/`Deprecated`). The command MUST list only flags currently in the registry (removed flags MUST NOT appear), and MUST NOT reflect run-specific activation state.
