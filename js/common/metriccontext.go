...
// Run executes fn under the captured context and restores the interrupted context afterwards.
func (c CapturedMetricContext) Run(fn func() error) error {
	if c.state == nil {
		return fn()
	}
	restore := c.Enter()
...
// SnapshotMetricContext captures the current tags and metadata for a callback that runs outside
// Sobek's promise machinery. Applying the snapshot isolates callback mutations from the interrupted
// context while allowing asynchronous work created by the callback to inherit those mutations.
func SnapshotMetricContext(state *lib.State) func() (restore func()) {
	captured := CaptureMetricContext(state)
	return captured.Enter
}
