...
func (ctx *tc39TestCtx) compile(base, name string) (*goja.Program, error) {
	...
	if prg == nil {
		...
		str := string(b)
		prg, _, err = ctx.compiler.Compile(str, name, true, compiler.CompilerOptions{Strict: false, CompatibilityMode: lib.CompatibilityModeExtended})
		if err != nil {
			return nil, err
		}
...
func (ctx *tc39TestCtx) runTC39Script(name, src string, includes []string, vm *goja.Runtime) (early bool, origErr, err error) {
	...
	var p *goja.Program
	p, _, origErr = ctx.compiler.Compile(src, name, true, compiler.CompilerOptions{Strict: false, CompatibilityMode: lib.CompatibilityModeBase})
	if origErr != nil {
		src, _, err = ctx.compiler.Transform(src, name, nil)
		if err == nil {
			p, _, err = ctx.compiler.Compile(src, name, true, compiler.CompilerOptions{Strict: false, CompatibilityMode: lib.CompatibilityModeBase})
		}
	} else {
		err = origErr
	}
