...
func (c *Compiler) parseImpl(src, filename string, wrap bool) (*ast.Program, bool, error) {
	code := src
	state := compilationState{loader: c.Options.SourceMapLoader, wrapped: wrap}
...
func (c *Compiler) compileImpl(
	...
	code := src
	state := compilationState{loader: c.Options.SourceMapLoader, wrapped: wrap, srcMap: srcMap}
	if state.wrapped { // the lines in the sourcemap (if available) will be fixed by increaseMappingsByOne
		conditionalNewLine := ""
		if index := strings.LastIndex(code, "//# sourceMappingURL="); index != -1 {
			// the lines in the sourcemap (if available) will be fixed by increaseMappingsByOne
