...
func TestAdd(t *testing.T) {
	c := Collector{}
	stat := Stat{Name: "test"}
	c.Add(Sample{Stat: &stat, Values: Values{"value": 12345}})
...
func TestAddNoStat(t *testing.T) {
	c := Collector{}
	c.Add(Sample{Values: Values{"value": 12345}})
	assert.Equal(t, 0, len(c.Batch))
}

func TestAddNoValues(t *testing.T) {
	c := Collector{}
	c.Add(Sample{Stat: &Stat{Name: "test"}})
	assert.Equal(t, 0, len(c.Batch))
}

func TestAddFixesTime(t *testing.T) {
	c := Collector{}
	c.Add(Sample{Stat: &Stat{Name: "test"}, Values: Values{"value": 12345}})
