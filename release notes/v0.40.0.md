k6 v0.40.0 is here!  This release includes the promised experimental modules, some small fixes, and a lot of changes needed for the metric time series, unindexable tags, and native ESM support. The [Roadmap and future plans section](#roadmap-and-future-plans) makes another appearance with more info on how things are going and what should be expected in the next releases.

# Breaking changes

- [#2632](https://github.com/grafana/k6/pull/2632) During the refactoring to set tags to `metric.add` in the order they are provided, we discovered that you can provide tags as key-value pair map *multiple* times in the same call. This has never been the intended use and was never documented. As it was undocumented, and as such functionality makes no sense alongside every other API k6 has, we decided to remove this ability.
- [#2582](https://github.com/grafana/k6/pull/2582) Previously, if `RegisterCallback` result was called twice, the second call would silently break the event loop. This has never been expected behavior, and calling it twice is always a bug in the code using it. Now, calling the `RegisterCallback` result twice leads to a panic.
...
The [xk6-redis](https://github.com/grafana/xk6-redis) has more examples, and the module is [documented in the official k6 documentation](https://k6.io/docs/javascript-api/k6-experimental-redis/).
...
This showcases how 1 VU can run multiple WebSockets connections asynchronously and how to stop them after a period using the `setTimeout` and friends.
...
Note that no k6 iterations finish if any WebSocket is still open or if a timeout orinterval is not cleared or triggered. This means that your script must take care of clearing all intervals and closing the WebSocket at some point. However, k6 still kills the whole process if it takes too long to stop after the maximum test duration is reached.
...
- Fixed a case where interrupting the VM did not work, especially around try/catch usage. This was particularly problematic for k6, as it could lead to k6 hanging. [#2600](https://github.com/grafana/k6/pull/2600)
...
The native ESM support is coming. A [PR to k6](https://github.com/grafana/k6/pull/2563) is under development, and a branch that will become a PR to goja to add the support there. The k6 team is hopeful that this will land in the next version, v0.41.0!
