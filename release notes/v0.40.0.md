...
## Main module/script no longer taints the global scope [#2571](https://github.com/grafana/k6/pull/2571)
...
`k6/http` has been using the [`throw` option](https://k6.io/docs/using-k6/k6-options/reference/#throw) to figure out if it should throw an exception on errors or whether it should return a response object with an error set on it (and log it).
...
This also is a minor breaking change as, by default, `throw` is `false`, so it will now no longer throw an exception, but instead now, it will return a Response with `error` property.
...
Here is a fairly big example showcasing using [xk6-redis](https://github.com/grafana/xk6-redis), but as an experimental module, to keep track of data in redis.
...
It is important to note that no k6 iterations do not finish if any WebSocket is still open or if a timeout/interval is not cleared or triggered. This means that you must be careful when writing the script to clear all intervals and close the WebSocket at some point. k6 will still kill the whole process if it takes too long to stop after the maximum test duration is reached.
...
Due to this fairly significant change, there are chances that some code will now behave differently. Please report any issues, though consider that it's possible that the new behavior is just the correct one.
...
While we are developing extensions internally, it constantly comes up that you need to create the same structure over and over again when you run stuff in tests. With the addition of the event loops, it is now required to setup it as well. Even k6 team members get parts of this wrong every once in a while, so we added a small type to be used by (extension) module developers to write tests easier. [#2602](https://github.com/grafana/k6/pull/2602)
...
This section will talk about our plans for future versions. You will notice that two of the big ticket items are here again. This is mostly due to the sheer size of the work required on both and some unforeseen needed changes - most of them actually making this better in the long run. It also so happens that it is vacation season so the k6 team rightfully is taking some personal time away.

## Native support for ECMAScript modules

The native ESM support is coming. There is both a fairly functioning (but still lacking in some areas) PoC [PR to k6](https://github.com/grafana/k6/pull/2563) and a branch that will (hopefully) soon become a PR to goja to actually add the support there.

It turned out that both there were a lot more things to be added as functionality - dynamic import, but also the [tc39](https://tc39.es/) group released released the latest ECMAScript specification adding support top level await in modules. While neither k6 nor goja has support for the `async`/`await` syntax, yet, this changes significantly the internal of the change which did require a not insignifcant refactor.
...
The k6 team is hopeful that this will land in the next version v0.41.0!

A future breaking change (⚠) is that it will no longer be possible to be using CommonJS style exports along import/export syntax in the *same* file.
...
It will still be possible to import a CommonJS module and `require` a ES module, or use `require` in an ES module. Having a dependency cycle mixing and matching CommonJS and ESM is also unlikely to work properly, but *might* do so in particular cases.

This is really expected to have never been done by anyone as there isn't really much of a reason for this, but is currently supported due to babel transpiling everything to CommonJS behind the scenes.

## Refactoring metrics

The refactoring of metrics has been under way and there are both a [PR](https://github.com/grafana/k6/pull/2594) for a more performant internal representation. This unfortunately took longer to get to a stable state and given the size and the type of change the k6 team decided to *not* release it very near the end of the cycle. It also doesn't have any noticeable change for most users. Instead of that it will be merged early on in the v0.41.0 release cycle and then more changes to make use of it will be made through the release cycle.

Some of hose changes include adding support for non-indexable tags which also has a read [PR](https://github.com/grafana/k6/pull/2654). This change also is really important for historical reasons connected with how `name` the tag works. As such it also was going to be needed to be merged in order to release the internal metric refactor.

⚠ Future breaking change: As part of the many changes and experiments we found out that we can keep `url` as indexable tag. Previously the plan is that it *along* `vu` and `iter` will become non-indexable. But the `url` tag is *heavily* used and enabled by default. Because of that (and other internal reasons) the new change will be that `url` will stay indexable, but if used with `name` will be overwritten to have the same value as `name`. Arguably this is what most users would want in the case when they are using `name`. For the ones that do not we plan on adding a *non* indexable `raw_url` tag. As such we no longer will be [printing a warning when `url` is used](https://github.com/grafana/k6/pull/2655) in thresholds.

Even as such we did make a bunch of changes to the internals of k6 that will pave the way forward. [#2629](https://github.com/grafana/k6/pull/2629), [#2631](https://github.com/grafana/k6/pull/2631).

[Prometheus remote-write output extension](https://github.com/grafana/xk6-output-prometheus-remote/) has not been worked on as all the changes required for it are dependent on the refactoring of metrics.

[Distributed racing support](https://github.com/grafana/k6/issues/2128) is itself needing the non indexable tag support. Once that is merged more work in that direction will be started.
