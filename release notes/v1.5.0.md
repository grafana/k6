...
The browser module now supports [`locator.pressSequentially()`](https://grafana.com/docs/k6/latest/javascript-api/k6-browser/locator/presssequentially/), which types text character by character, simulating real keyboard input.

This method is useful for testing features that require gradual typing to trigger specific behaviors, such as autocomplete suggestions, input validation per character, or character counters. Unlike [`locator.fill()`](https://grafana.com/docs/k6/latest/javascript-api/k6-browser/locator/fill/), which fills text instantly, `pressSequentially()` fires keyboard events (`keydown`, `keypress`/`input`, and `keyup`) for each character.

The method supports configurable delay between keystrokes:
