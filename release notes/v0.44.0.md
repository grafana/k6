...
- A new experimental Web Crypto module.
...
- [xk6-browser#790](https://github.com/grafana/xk6-browser/pull/790) Converted `locator.click` to async to have feature parity with `page.click` and `elementHandle.click`. Users must remember to work with `promise.All` and `page.waitForNavigation()` when a click action results in navigation.
...
- [xk6-browser#844](https://github.com/grafana/xk6-browser/pull/844) Removed the exported `version` param from the root module. Users should from now on reference the k6 version instead of the k6-browser module version.
- [xk6-browser#838](https://github.com/grafana/xk6-browser/pull/838) Removed first meaningful paint metric. This metric is being deprecated across all the browsers, so we've removed it.

## New features

### A new experimental Web Crypto module [#2884](https://github.com/grafana/k6/pull/3007)

This release includes new experimental Web Crypto module. This is a work in progress implementation of the [Web Crypto API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Crypto_API) specification for k6.
...
- [xk6-browser#788](https://github.com/grafana/xk6-browser/pull/788) Updated the xk6-browser readme to highlight that it is now a module in k6.
- [xk6-browser#803](https://github.com/grafana/xk6-browser/pull/803) Users are now warned if the `browser.close` method is called more than once.
- [xk6-browser#820](https://github.com/grafana/xk6-browser/pull/820) Added error handling to wildcard selectors, which cleans up the error output in the terminal.
- [xk6-browser#843](https://github.com/grafana/xk6-browser/pull/843) Removed the build step from github actions. From this release onwards, no new standalone xk6-browser binaries will be built and available from the [releases](https://github.com/grafana/xk6-browser/releases) section. The latest version of the xk6-browser module will be available in the k6 binary which can be found [here](https://github.com/grafana/k6/releases).
- [xk6-browser#848](https://github.com/grafana/xk6-browser/pull/848) Multiple k6 instances can now connect to one browser to run concurrent tests. This update empowers high-concurrency k6 browser testing with multiple VUs and instances. Using the new `browserType.Connect` API, users can now connect to an existing browser instance and execute concurrent tests, which was not possible previously.
...
- [xk6-browser#781](https://github.com/grafana/xk6-browser/pull/781) Fixed mapping of `response` object's function from `jSON` to `json`.
- [xk6-browser#779](https://github.com/grafana/xk6-browser/pull/779) Cleared Zombie processes on panic.
- [xk6-browser#834](https://github.com/grafana/xk6-browser/pull/834) Fixed `page.close` so that it closes the current page and not the whole browser context.

## Maintenance and internal improvements

### Improved the per-VU buffer pool [#2879](https://github.com/grafana/k6/pull/2879)

Improved the per-VU buffer pool which should reduce memory usage and improve performance. For some test cases, this change can [reduce memory usage up to 50%](https://github.com/grafana/k6/issues/794#issuecomment-1406522459).
...
- [xk6-browser#776](https://github.com/grafana/xk6-browser/pull/776) Fixed a test for preset flags for chrome on darwin based systems.
- [xk6-browser#782](https://github.com/grafana/xk6-browser/pull/782) Refactored a mapping of "special" functions to work with a `map`.
- [xk6-browser#783](https://github.com/grafana/xk6-browser/pull/783) Fixed a mapping by unmapping internal methods.
- [xk6-browser#797](https://github.com/grafana/xk6-browser/pull/797) Fixed multi browser close.
- [xk6-browser#796](https://github.com/grafana/xk6-browser/pull/796) An error is returned from `browserContext.SetExtraHTTPHeaders` and any panics are handled in the mapping layer.
- [xk6-browser#798](https://github.com/grafana/xk6-browser/pull/798) Added a locator mapping test.
- [xk6-browser#799](https://github.com/grafana/xk6-browser/pull/799) Added a test to detect redundant mappings.
- [xk6-browser#802](https://github.com/grafana/xk6-browser/pull/802) Added a helper to assert on the logs and dump the logs for easier debugging.
- [xk6-browser#807](https://github.com/grafana/xk6-browser/pull/807) Fixed incorrect keyboard key code on up/down key presses.
- [xk6-browser#810](https://github.com/grafana/xk6-browser/pull/810) Renamed ErrInternal to ErrFatal and used abort/interrupt.
- [xk6-browser#821](https://github.com/grafana/xk6-browser/pull/821) Upgraded to k6 v0.43.1.
- [xk6-browser#824](https://github.com/grafana/xk6-browser/pull/824) Refactored logger.New to accept FieldLogger.
- [xk6-browser#826](https://github.com/grafana/xk6-browser/pull/826) Fixed Edge-case: Mapping used to fail if the extension code returns nil values.
- [xk6-browser#830](https://github.com/grafana/xk6-browser/pull/830) Updated to k6-core master and fixed BPool.
- [xk6-browser#832](https://github.com/grafana/xk6-browser/pull/832) Fixed the killing of a browser process on panic if a process couldn't be found with a pid.
- [xk6-browser#819](https://github.com/grafana/xk6-browser/pull/819) Browser.Launch now transitions to Browser.Connect when a CDP url is provided in an env var.
