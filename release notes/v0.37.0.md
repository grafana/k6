...
It's important to highlight that the old API (e.g. methods like `context.WithRuntime`, `common.Bind` and others [#2384](https://github.com/grafana/k6/pull/2384)) is deprecated and will be removed in the next k6's release (`v0.38.0`). Every extension that isn't using that API will get a warning message, like:
...
We continue pushing the images to the `loadimpact/k6` for several more releases, but if you use it in your local or CI environments please plan the migration.
