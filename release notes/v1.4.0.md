...
- [#5320](https://github.com/grafana/k6/pull/5320), [#5239](https://github.com/grafana/k6/pull/5239), [#5342](https://github.com/grafana/k6/pull/5342) Automatic extension resolution now only inspects ES module `import` statements and no longer supports CommonJS `require()` calls. CommonJS `require()` calls are dynamic and it is not possible to know for certain if they will be called, or if they will be called with static strings - the only way they were even previously loaded. This functionality was a quirk of the previous implementation and had numberous problems. Additionally, `use k6` directives are now only recognized when they appear at the beginning of files (after optional shebang and whitespace/comments). This was the initial intention, but due to implementation bugs it was not actually was happening.
...
### Automatic extension resolution improvements [#5320](https://github.com/grafana/k6/pull/5320), [#5239](https://github.com/grafana/k6/pull/5239), [#5342](https://github.com/grafana/k6/pull/5342)
...
1. trying to follow `require` calls, which due to their dynamic nature doesn't work particularly stable. That is depending on where and how `require` call was used it was possible that k6 might decided it is needed or not. And it definitely doesn't work wiht actually using string variables. Support for CommonJS is mostly for backwards compatibiliy, so after internal discussion we opted to not support this at all. We could bring back this until v2, if there is enough interest. But long term it is inteded for this to not be part of k6.
2. "use k6 with ..." directives, were parsed from the whole file instead of just the beginning, which leads to numerous problems, and was not the intented case. As such they are now only parsed at the beginning of files (not only the main one) with potential empty lines and comments infront.
...
```
Or an axample using the directive but commonJS
```javascript
// utils.js
...
2. Syntax error were not reported as such as the underlying esbuild parsing will fail but won't be handled well. [#5127](https://github.com/grafana/k6/issues/5127), [#5104](https://github.com/grafana/k6/issues/5104)

3. Propagating exit codes from sub-process running the new k6. This lets you use the result of the exit code.
...
- [#5159](https://github.com/grafana/k6/pull/5159) Simplifies warning message for legacy config file.
...
- [#5313](https://github.com/grafana/k6/pull/5313) Update to k6provider v0.2.0 and update code to use the new API.
