...
### Cloud execution logs ([#1599](https://github.com/k6io/k6/pull/1599))
...
### Pushing k6 logs to loki ([#1576](https://github.com/k6io/k6/pull/1576))
...
### Optional port to host mappings ([#1489](https://github.com/k6io/k6/pull/1489))
...
### Support for specifying data types to InfluxDB fields ([#1395](https://github.com/k6io/k6/pull/1395))
...
### Support for automatic gzip-ing of the CSV output result ([#1566](https://github.com/k6io/k6/pull/1566))
...
- Various spacing and progress bar rendering issues were improved ([#1580](https://github.com/k6io/k6/issues/1580)).
- The k6 ASCII logo was made a bit more proportional ([#1615](https://github.com/k6io/k6/pull/1615/commits/54a3168f0945fa8243edb7540fe78ad7ac66a8dc)). Thanks, @rawtaz!
- The docker-compose example setup from the k6 repo now contains a built-in simple dashboard ([#1610](https://github.com/k6io/k6/pull/1610)). Thanks, @jeevananthank!
...
- Network: IPv6 support was fixed as a part of the new `hosts` port mapping ([#1489](https://github.com/k6io/k6/pull/1489)). Thanks, @calavera!
- Metrics: Fixed the wrong `name` metric tag for redirected requests ([#1474](https://github.com/k6io/k6/issues/1474)).
- UI: Fixed a `divide by zero` panic caused by some unusual execution environments that present a TTY, but return `0` for the terminal size ([#1581](https://github.com/k6io/k6/pull/1581)).
- Config: Fixed the parsing of `K6_DATADOG_TAG_BLACKLIST` ([#1602](https://github.com/k6io/k6/issues/1602)).
- Config: Fixed marshaling of `tlsCipherSuites` and `tlsVersion` ([#1603](https://github.com/k6io/k6/pull/1603)). Thanks, @berndhartzer!
- WebSockets: Fixed a `ws.SetTimeout()` and `ws.SetInterval()` panic when float values were passed ([#1608](https://github.com/k6io/k6/pull/1608)).

## Internals

- goja, the JavaScript runtime k6 uses, was updated to the latest version. This means that k6 with `--compatibility-mode=base` now supports some standard library features from ES6 ([goja's PR](https://github.com/dop251/goja/pull/180)), though no new syntax yet. In future versions we plan to drop some current core.js modules that are no longer needed, which should greatly reduce memory usage per VU ([#1588](https://github.com/k6io/k6/pull/1588)).
- Go modules are now used to manage the k6 dependencies instead of `dep` ([#1584](https://github.com/k6io/k6/pull/1584)).

## Breaking changes

- `k6 cloud` will now proxy execution logs back to the client machine. To disable this behavior, use `k6 cloud --show-logs=false`.

- `--http-debug` request and response dumps are now emitted through the logging sub-system, to facilitate the cloud log proxying ([#1577](https://github.com/k6io/k6/pull/1577)).
