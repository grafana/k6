...
These new scenarios are entirely optional, and existing k6 scripts and options should continue to work the same as before (with a few minor [breaking changes](#breaking-changes)), but please [create a bug issue](https://github.com/loadimpact/k6/issues/new?labels=bug&template=bug_report.md) if that's not the case.
...
- k6 will now emit a new `dropped_iterations` metric in `shared-iterations`, `per-vu-iterations`, `constant-arrival-rate` and `ramping-arrival-rate` executors if it can't schedule an iteration on time, depending on the configured rates and the iteration duration ([#1529](https://github.com/loadimpact/k6/pull/1529)).
