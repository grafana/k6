...
TODO: example with 2 scenarios, for example constant-vus to test a website and arrival-rate for an api test

As shown in the example above and the [documentation](https://k6.io/docs/using-k6/executors), all executors have some additional options that improve their flexibility and facilitate code reuse, especially in multi-scenario test runs:
...
- Execution: Fixed a bug that caused k6 to skip some iterations with a `context cancelled` error when rapidly ramping up and down
