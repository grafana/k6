...
### Stack ID is now required for all `k6 cloud` commands [#5833](https://github.com/grafana/k6/pull/5833)

Providing a stack is now mandatory for all `k6 cloud` commands (`k6 cloud run`, `k6 cloud upload`, `k6 cloud run --local-execution`). Previously, omitting a stack would fall back to the first available stack with a deprecation warning — that fallback has been removed. Likewise, `k6 cloud login` now requires both a token and a stack; passing one without the other fails with an explicit error. The legacy v1 token-only validation path (`/v1/validate-token`) has also been removed.

**Migration:** Ensure a stack is always provided via `k6 cloud login --stack`, the `K6_CLOUD_STACK_ID` environment variable, or the `stackID` script option before running any `k6 cloud` command.
...
- [#5783](https://github.com/grafana/k6/pull/5783), [#5838](https://github.com/grafana/k6/pull/5838) Updates `github.com/grafana/k6-cloud-openapi-client-go` to API spec v1.9.1.
...
- [#5718](https://github.com/grafana/k6/pull/5718) Updates `github/codeql-action` to `v4.35.1`.
