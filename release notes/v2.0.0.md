...
- **_(new since v2.0.0-rc1)_** Cloud secrets are now automatically available in `k6 cloud run --local-execution` without requiring `--secret-source=cloud`.
...
A new built-in secret source, `cloud`, allows k6 scripts to retrieve secrets stored in Grafana Cloud directly during local execution. Thanks, @vortegatorres!
...
When running `k6 cloud run --local-execution`, the cloud secret source is now automatically available — no `--secret-source=cloud` flag is required. Cloud-managed secrets are accessible via `secrets.get()` out of the box.

```shell
# Cloud secrets are now available automatically
k6 cloud run --local-execution script.js
```

To opt out, use `--secret-source` explicitly with a different source.
...
- _(new since v2.0.0-rc1)_ Bumps `github.com/grafana/k6provider` to `v0.5.0`.
- _(new since v2.0.0-rc1)_ Uses `synctest` in `PeriodicFlusher`, `output/cloud`, and `output/cloud/expv2` tests for faster, more deterministic CI runs.
- _(new since v2.0.0-rc1)_ Sets `K6ModPath` to the v2 module path in the k6provider config so that k6 v2 correctly signals to k6build which module to resolve against (`go.k6.io/k6/v2`).
- _(new since v2.0.0-rc1)_ [#5911](https://github.com/grafana/k6/pull/5911) Updates `go.opentelemetry.io/otel` packages.
