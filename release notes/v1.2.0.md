...
We have made multiple enhancements to the binary provisioning feature:
...
- [#4812](https://github.com/grafana/k6/pull/4812) Considers built-in extensions before attempting binary provisioning. When running a test using a custom `k6` binary, the binary provisioning checks if the binary includes all required extensions.
' [#4921](https://github.com/grafana/k6/pull/4921) Removes the requirement for GCK6 authentication for binary provisioning, and enables the binary provisioning feature for local execution (i.e., the `k6 run` command).
...
```
K6_BINARY_PROVISIONING=true K6_ENABLE_COMMUNITY_EXTENSIONS=true k6 run script.js
```
...
Previously, you would have to write boilerplate code to perform a health check:
```javascript
...
```
This enhancement was contributed by @tbourrely. Thank you for your contribution!
...
- [#4958](https://github.com/grafana/k6/pull/4958) Binary provisioning: pass k6's file system to k6deps
