...
k6 extensions allow you to add custom functionality to your performance tests, such as connecting to databases, message queues, or specialized networking protocols. Previously, using extensions [required manual building](https://grafana.com/docs/k6/latest/extensions/build-k6-binary-using-go) of a custom k6 binary with the extensions compiled in. This new version introduces the _Automatic Extension Resolution_ functionality, which is enabled by default and automatically detects when your script imports extensions and handles the complexity of provisioning the right k6 binary for you.

The previous experimental versions only supported official extensions. [#4922](https://github.com/grafana/k6/pull/4922) added the support to use any extension listed in the [extensions registry](https://grafana.com/docs/k6/latest/extensions/explore/) by setting the `K6_ENABLE_COMMUNITY_EXTENSIONS` environment variable.
...
- [#5019](https://github.com/grafana/k6/pull/5019) The feature is now enabled by default. The environment variable has been renamed to `K6_AUTO_EXTENSION_RESOLUTION` to better reflect the feature's name and is now used for disabling the functionality when needed.
- [#4811](https://github.com/grafana/k6/pull/4811) Removes limitations that previously prevented processing test scripts from stdin or using file extensions other than `.js` or `.tar`.
- [#4812](https://github.com/grafana/k6/pull/4812) When running a test using a custom `k6` binary, the automatic extension resolution checks if the it includes all required extensions before attempting to provision a custom binary.
' [#4921](https://github.com/grafana/k6/pull/4921) Removes the requirement for GCK6 authentication for using the automatic extension resolution, and enables the feature for local execution (i.e., the `k6 run` command).
- [#4922](https://github.com/grafana/k6/pull/4922) Supports running tests using community extensions.
...
The k6 gRPC module now includes a `client.healthCheck()` method, which provides a convenient way to check the health of a gRPC service without needing to manually craft `invoke` calls. This simplifies the process of checking whether a service is up and running, which is particularly useful for readiness checks and service discovery.
...
The method returns a response object that includes the `status` of the service. You can use the new `grpc.HealthCheckStatus` constants to interpret the response:
- `grpc.HealthCheckStatus.SERVING`
- `grpc.HealthCheckStatus.NOT_SERVING`
- `grpc.HealthCheckStatus.UNKNOWN`
...
export default () => {
  ...
  check(response, {
    'health check status is SERVING': (r) => r.status === grpc.HealthCheckStatus.SERVING,
  });
