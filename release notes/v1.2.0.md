...
We have made multiple enhancements for running scripts with extensions:

- [#5019](https://github.com/grafana/k6/pull/5019) Enables the feature by default. Also renames it as Automatic Extension Resolution and the environment variable to enables/disables it as `K6_AUTO_EXTENSION_RESOLUTION`.
- [#4811](https://github.com/grafana/k6/pull/4811) Removes limitations for processing test scripts from stdin or using file extensions other than `.js` or `.tar`.
...
Previously, k6 only supported official extensions. As of the PR [#4922](https://github.com/grafana/k6/pull/4922), it's now possible to use any extension listed in the [extensions registry](https://grafana.com/docs/k6/latest/extensions/explore/) by setting the `K6_ENABLE_COMMUNITY_EXTENSIONS` environment variable.

```
K6_ENABLE_COMMUNITY_EXTENSIONS=true k6 run script.js
```

Note: Community extensions are supported only for local executions (`k6 run` or `k6 cloud run --local-execution`). Tests executed in GCK6 still are limited to use the official extensions.
