...
- [#4812](https://github.com/grafana/k6/pull/4812) Consider built-in extensions before attempting binary provisioning. When running a test using a custom `k6` binary, the binary provisioning will check if it provides all required extensions.
- [#4921](https://github.com/grafana/k6/pull/4921) GCK6 authentication is no longer required for using binary provisioning. This also makes binary provisioning available for `k6 run` command (local execution).
- [#4922](https://github.com/grafana/k6/pull/4922) Supports running tests using community extensions.

Previously to this release, only official extensions were supported. Since [#4922](https://github.com/grafana/k6/pull/4922) it is possible to opt-in for using any extension listed in the [extensions registry](https://grafana.com/docs/k6/latest/extensions/explore/) with the `K6_ENABLE_COMMUNITY_EXTENSIONS` environment variable.
