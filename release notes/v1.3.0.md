...
### Locator filtering [#5114](https://github.com/grafana/k6/pull/5114), [#5131](https://github.com/grafana/k6/pull/5131), [#5150](https://github.com/grafana/k6/pull/5150)

**`locator.filter()`**
The browser module now supports `locator.filter()`, which creates a new `locator` that matches only elements containing or excluding specified text. This method enables precise element filtering within existing `locator` results, making it easier to target specific elements when multiple matches exist. We'll continue to add other filtering options in future releases, such as filtering by child `locator`s.
...
**Enhanced locator methods with filtering options**
We've also added filtering options support to locator creation methods across all contexts:

**`page.locator(selector, options)`** - Create page locators with optional text filtering.
```javascript
...
**`frame.locator(selector, options)`** - Create frame locators with optional text filtering.
```javascript
...
**`locator.locator(selector, options)`** - Chain locators with optional text filtering.
```javascript
...
This comprehensive filtering system provides more reliable element targeting and works seamlessly across all locator contexts.
