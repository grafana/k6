...
- New `k6 deps` command for analyzing script dependencies and required extensions.
- Browser module enhancements with `frameLocator()` method and new request event handlers.
- Cloud commands now support configurable default stacks for multi-region deployments.
- Crypto module adds PBKDF2 support for password-based key derivation.
- Improved console logging for ArrayBuffer and TypedArray types.
- Experimental Prometheus output now supports configurable TLS versions.
...
The command identifies all imports in your script and lists dependencies that might be needed for building a new binary with auto extension resolution. Similar to auto extension resolution itself, this only accounts for imports, not dynamic `require()` calls.
...
In addition k6 now supports manifest that will set a default version for any dependency that doesn't have set one. It will not in itself request any dependency, just set versions for ones with "*" as the constraint.

The manifest is set through env variable that is JSON with keys being a dependncy and the value being a contraint:

```
K6_DEPENDENCIES_MANIFEST='{"k6/x/faker": ">=v0.4.4"}' k6 run scripts.js
```

In this example if the scripts just imports `k6/x/faker` and does not use a `use k6 with k6/x/faker ...` directive it will set the version constraint to `>=v0.4.4`. It will not do any changes if `k6/x/faker` is not dependency of the script at all.
...
The `requestfailed` event fires when a request fails (network errors, aborts, etc.), while `requestfinished` fires when a request completes successfully. These events are essential for:

- Debugging failed network requests in browser tests
- Validating that specific API calls complete successfully
- Monitoring request timing and performance
- Detecting and handling network errors gracefully
...
### Cloud commands: Configurable default stack [#5420](https://github.com/grafana/k6/pull/5420)

Cloud commands now support configuring the default Grafana Cloud stack, which identifies your specific Grafana Cloud stack. The stack slug is used by the cloud to determine which default project to use when not explicitly provided.
...
The stack can be configured via:
- `k6 cloud login` command with the stack option
- Environment variable (`K6_CLOUD_STACK_URL`)
- Configuration file

```bash
# Login and set default stack
k6 cloud login --stack-url https://ingest.your-stack.grafana.net

# Run test using the configured default stack
k6 cloud run script.js

# Project ID in script options takes precedence
...
export const options = {
  cloud: {
    projectID: 789,  // This overrides the default stack's project
  },
};
```
...
The crypto module now supports PBKDF2 (Password-Based Key Derivation Function 2) for deriving cryptographic keys from passwords. PBKDF2 is widely used for password hashing and key derivation in security-sensitive applications, and this addition enables testing of systems that use PBKDF2 for authentication or encryption.

This is particularly useful for:
- Testing password hashing implementations
- Generating keys for encrypted test data
- Validating PBKDF2-based authentication systems
- Creating realistic test scenarios for security-sensitive applications

```javascript
import crypto from 'k6/crypto';
import { encodingToArrayBuffer } from 'k6/encoding';
...
const float64 = new Float64Array([3.14, 2.71, 1.41]);
console.log(float64);
// Output: Float64Array(3) [3.14, 2.71, 1.41]
```

This improvement significantly enhances the debugging experience when working with binary data, WebSockets, or any protocol that uses typed arrays.

### Experimental Prometheus output: Configurable TLS version [#5537](https://github.com/grafana/k6/pull/5537)
...
```bash
# Configure minimum TLS version via environment variable
K6_PROMETHEUS_RW_TLS_MIN_VERSION=1.3 k6 run script.js -o experimental-prometheus-rw

# Or via config file
k6 run script.js -o experimental-prometheus-rw --config prometheus.json
```

This is particularly useful when working with security-hardened infrastructure that requires specific TLS versions.

## UX improvements and enhancements

- [#5538](https://github.com/grafana/k6/pull/5538) Adds TOTP with secrets example for demonstrating time-based one-time password generation.

## Bug fixes

- [#5546](https://github.com/grafana/k6/pull/5546) Fixes JSON error position reporting in HTTP responses, providing accurate line and character numbers for syntax errors.
- [#5526](https://github.com/grafana/k6/pull/5526) Fixes browser module ignoring context cancellation during target attachment, preventing unnecessary error logs. Closes [#5504](https://github.com/grafana/k6/issues/5504).
...
- [#5132](https://github.com/grafana/k6/pull/5132) Fixes missing exception when using invalid salt length with RSA encryption.
