...
func TestRuntimeOptions(t *testing.T) {
	...
	var (
		...
		defaultTracing                   = null.NewString("none", false)
	...
	runtimeOptionsTestCases := map[string]runtimeOptionsTestCase{
		...
		"tracing configuration from flags": {
			useSysEnv: false,
			cliFlags: []string{
				"--tracing", "http", "--traces-parent", "parent", "--traces-sampler", "traceidratio",
				"--traces-sampler-arg", "0.25", "--traces-propagator", "jaeger",
			},
			expRTOpts: lib.RuntimeOptions{
				IncludeSystemEnvVars:      null.NewBool(false, false),
				CompatibilityMode:         defaultCompatMode,
				Env:                       map[string]string{},
				Tracing:                   null.NewString("http", true),
				TracesOutput:              defaultTracesOutput,
				TracesParent:              null.StringFrom("parent"),
				TracesSampler:             null.StringFrom("traceidratio"),
				TracesSamplerArg:          null.StringFrom("0.25"),
				TracesPropagator:          null.StringFrom("jaeger"),
				SummaryMode:               defaultSummaryMode,
				NewMachineReadableSummary: defaultNewMachineReadableSummary,
			},
		},
		"tracing configuration from env": {
			useSysEnv: false,
			systemEnv: map[string]string{
				"K6_TRACING":            "http",
				"K6_TRACES_PARENT":      "parent",
				"K6_TRACES_SAMPLER":     "traceidratio",
				"K6_TRACES_SAMPLER_ARG": "0.5",
				"K6_TRACES_PROPAGATOR":  "jaeger",
			},
			expRTOpts: lib.RuntimeOptions{
				IncludeSystemEnvVars:      null.NewBool(false, false),
				CompatibilityMode:         defaultCompatMode,
				...
				TracesParent:              null.StringFrom("parent"),
				TracesSampler:             null.StringFrom("traceidratio"),
				TracesSamplerArg:          null.StringFrom("0.5"),
				TracesPropagator:          null.StringFrom("jaeger"),
				SummaryMode:               defaultSummaryMode,
				NewMachineReadableSummary: defaultNewMachineReadableSummary,
			},
		},
		"traces split from flags": {
			useSysEnv: false,
			cliFlags:  []string{"--traces-split"},
			expRTOpts: lib.RuntimeOptions{
				IncludeSystemEnvVars:      null.NewBool(false, false),
				CompatibilityMode:         defaultCompatMode,
				Env:                       map[string]string{},
				Tracing:                   defaultTracing,
				TracesOutput:              defaultTracesOutput,
				TracesSplit:               null.NewBool(true, true),
				SummaryMode:               defaultSummaryMode,
				NewMachineReadableSummary: defaultNewMachineReadableSummary,
			},
		},
		"traces split from env": {
			useSysEnv: false,
			systemEnv: map[string]string{"K6_TRACES_SPLIT": "true"},
			expRTOpts: lib.RuntimeOptions{
				IncludeSystemEnvVars:      null.NewBool(false, false),
				CompatibilityMode:         defaultCompatMode,
				Env:                       map[string]string{},
				Tracing:                   defaultTracing,
				TracesOutput:              defaultTracesOutput,
				TracesSplit:               null.BoolFrom(true),
				SummaryMode:               defaultSummaryMode,
				NewMachineReadableSummary: defaultNewMachineReadableSummary,
			},
		},
		"traces split env var error detected even when CLI flags overwrite": {
			useSysEnv: false,
			systemEnv: map[string]string{"K6_TRACES_SPLIT": "boo"},
			cliFlags:  []string{"--traces-split"},
			expErr:    true,
		},
