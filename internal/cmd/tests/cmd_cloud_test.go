...
func TestCloudDirectInvocationShowsHelp(t *testing.T) {
	t.Parallel()

	ts := NewGlobalTestState(t)
	require.NoError(t, fsext.WriteFile(ts.FS, filepath.Join(ts.Cwd, "test.js"), []byte("export default function() {}"), 0o644))
	ts.CmdArgs = []string{"k6", "cloud", "test.js"}
	// No cloud server set up — execution must NOT occur
	cmd.ExecuteWithGlobalState(ts.GlobalState)

	stdout := ts.Stdout.String()
	t.Log(stdout)
	assert.Contains(t, stdout, "Run and manage", "expected help text to be shown")
	assert.NotContains(t, stdout, "execution: cloud", "expected no test execution")
	assert.NotContains(t, stdout, "[deprecated]", "expected no deprecated examples in help output")
}
