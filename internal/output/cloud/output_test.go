...
func (m *provisioningNotifierMock) NotifyTestRunCompleted(_ context.Context, testRunID int32, token string, testErr error) error {
	m.called.Store(true)
	m.callCount.Add(1)
...
func TestOutputStopWithTestError_ProvisioningMode_NoError(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		assert.Equal(t, int32(12345), notifierMock.testRunID)
