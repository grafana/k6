...
// TestProvisionLocalExecution_ArchiveNilSkipsUploadAndPoll verifies that when Archive is nil,
// neither the S3 upload nor the polling endpoint is called.
func TestProvisionLocalExecution_ArchiveNilSkipsUploadAndPoll(t *testing.T) {
	t.Parallel()
	...
	assert.Equal(t, int32(0), pollCalls.Load(), "poll endpoint must NOT be called when Archive is nil")
}
