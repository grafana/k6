...
func (c *Client) ProvisionLocalExecution(ctx context.Context, params ProvisionParams) (*ProvisionResult, error) {
	...
	// Always poll: the backend may queue the test run regardless of whether
	// an archive was uploaded. archive_size=null signals no archive is expected,
	// so the backend skips waiting for upload and goes straight to validation,
	// but the test run may still be queued before becoming ready.
