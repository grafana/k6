...
// A synchronously throwing then() propagates its error and still records duration.
func TestGroupThenableThrowsStillEmitsDuration(t *testing.T) {
	t.Parallel()
	tc := testCaseRuntime(t)

	_, err := tc.testRuntime.RunOnEventLoop(
		`k6.group("g", () => { return { then: function () { throw new Error("boom"); } }; });`)
	require.Error(t, err) // the thenable's throw propagates
