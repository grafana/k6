...
type Page struct {
	...
	closeErr    error
	// closingCh is closed when the page begins tearing down.  Checked by
...
func (p *Page) waitForFrameSessions(ctx context.Context) error {
...
func (p *Page) Close() error {
	...
	p.closingOnce.Do(func() {
		...
		if err := add.Do(cdp.WithExecutor(teardownTimeoutCtx, p.session)); err != nil {
			p.closeErr = spanRecordErrorf(span, "internal error while removing binding from page: %w", err)
		}

		action := target.CloseTarget(p.targetID)
		err = action.Do(cdp.WithExecutor(teardownTimeoutCtx, p.session))
		if err != nil {
			...
			if !errors.Is(err, context.Canceled) {
				p.closeErr = spanRecordErrorf(span, "closing a page: %w", err)
			}
		}
		...
		p.cancelCtx()
		if err := p.waitForFrameSessions(teardownTimeoutCtx); err != nil {
			p.logger.Warnf("Page:Close", "timed out waiting for frame sessions: %v", err)
		}
	})
