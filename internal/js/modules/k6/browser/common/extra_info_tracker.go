...
// they arrive in order for a given requestId. In a redirect chain, one
// requestId produces multiple request/response pairs. We pair them by index:
// the i-th extraInfo event corresponds to the i-th request/response.
type requestExtraInfo struct {
	// Indexed queues of parsed extra headers. Slots are set to nil
	// after pairing to avoid applying headers twice.
	requestExtraHeaders  []map[string][]string
	responseExtraHeaders []map[string][]string

	// Indexed queues of requests/responses awaiting extra headers.
	// Slots are set to nil after pairing.
	requests  []*Request
	responses []*Response
}
