...
func (rsasv *RSAPssParams) Sign(key CryptoKey, data []byte) ([]byte, error) {
	...
	if rsasv.SaltLength == 0 {
		return nil, NewError(ImplementationError, "k6 RSA-PSS uses standard Golang SDK that doesn't support salt length=0 but generates salt with maximum length . Sign result might be different.")
	}
...
func (rsasv *RSAPssParams) Verify(key CryptoKey, signature []byte, data []byte) (bool, error) {
	...
	if rsasv.SaltLength == 0 {
		return false, NewError(ImplementationError, "k6 RSA-PSS uses standard Golang SDK that doesn't support salt length=0 but tries to auto-detect salt length if 0 is used. Verify result might be different.")
	}
