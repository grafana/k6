...
func TestCookieJar(t *testing.T) {
	...
	httpInstance := modulestest.WithInitContext(ts.VU, func() modules.Instance {
		return httpModule.New().NewModuleInstance(ts.VU)
	})
	err := ts.VU.Runtime().Set("http", httpInstance.Exports().Default)
	require.NoError(t, err)
