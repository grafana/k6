...
func (g *asyncGroup) run(thenFn sobek.Callable, thenable sobek.Value) (sobek.Value, error) {
	rt := g.vu.Runtime()
	if thenable.ExportType() == reflect.TypeFor[*sobek.Promise]() {
		return thenFn(
			thenable,
	...
	// A generic thenable is not required to return a Promise from then(). Assimilate its settlement
	// into a Sobek Promise so group() remains awaitable and inherits Promise resolution semantics,
	// including first-settlement-wins and recursively adopting another Promise.
	promise, resolve, reject := rt.NewPromise()
	settle := func(settler func(any) error) func(sobek.FunctionCall) sobek.Value {
		return func(call sobek.FunctionCall) sobek.Value {
			if err := settler(call.Argument(0)); err != nil {
				panic(err)
			}
	...
	_, err := thenFn(
		thenable,
		rt.ToValue(settle(resolve)),
		rt.ToValue(settle(reject)),
	)
	if err != nil {
		// A JavaScript throw while invoking then() rejects the assimilating Promise. If the thenable
		// already settled, Sobek's resolving functions ignore this later rejection.
		exception, ok := err.(*sobek.Exception) //nolint:errorlint // preserve the JavaScript value
		if !ok {
			return nil, err
		}
		if err = reject(exception.Value()); err != nil {
			return nil, err
		}
	}

	// Continue through the common settlement path: reject may have been ignored if the thenable
	// settled first, and the handlers below own duration emission.
	promiseValue := rt.ToValue(promise)
	promiseThen, ok := asThenable(promiseValue)
	if !ok {
		panic("new Sobek promise has no callable then method")
	}
	return promiseThen(
		promiseValue,
		rt.ToValue(g.onFulfilled),
