...
// Stop is deprecated, see the StopWithTestError() method.
func (out *Output) Stop() error {
	return out.StopWithTestError(nil)
}

// StopWithTestError gracefully stops all metric emission from the output and
// when all metric samples are emitted, it sends an API to the cloud to finish
// the test run. If testErr was specified, it extracts the RunStatus from it.
