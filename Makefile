...
# TODO: check if the image non exists and suggest running dev-tools
...
## dev-tools: Builds an image with all tools for the development.
dev-tools:
	docker build \
		--build-arg USER=$(USER) \
...
.PHONY: build format lint test check dev-tools generate fix
